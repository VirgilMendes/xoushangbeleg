	...
	int VueCombat::run()
	{
		...
		switch (event.type)
			...
			case sf::Event::KeyPressed: switch (event.key.code)
				{
				case sf::Keyboard::Z: curseur_.deplacerCurseur(sf::Vector2i(0, -1));
					break;
				case sf::Keyboard::Q: curseur_.deplacerCurseur(sf::Vector2i(-1, 0));
					break;
				case sf::Keyboard::S: curseur_.deplacerCurseur(sf::Vector2i(0, 1));
					break;
				case sf::Keyboard::D: curseur_.deplacerCurseur(sf::Vector2i(1, 0));
					break;
				default: break;
				}
			}
...
fenetre_->display();
	...
	void VueCombat::ajouterUnite(std::string nom, std::string cheminTexture, Modele::Vecteur2<int> position)
	{
		sf::Texture* texture;
		...
		else { texture = textures_[cheminTexture]; }
		unites_.push_back(Unite(nom, texture, position));
	}

	void VueCombat::deplacerUnite(std::string nom, Modele::Vecteur2<int> position)
	{
		Unite* unite = nullptr;
